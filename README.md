...
📖 **Full walkthrough:** [When an APEX App Talks Back — build it yourself](ADD-YOUR-BLOG-LINK-HERE)
...
├── sql/                  # schema, REST handlers, and seed data — run in numeric order
│   ├── 01_schema_ddl.sql
│   ├── 02_seed_data.sql
│   ├── 03_rest_api.sql
│   ├── 04_post_returns_endpoint.sql
│   ├── 05_apex_page_queries.sql
│   ├── 06_approval_flow.sql
│   ├── 07_update_inbound_message_endpoint.sql
│   ├── 08_additional_seed_data.sql
│   ├── 09_data_reporter_setup.sql
│   └── 10_seed_date.sql
├── n8n/
│   ├── RMA_Copilot_v2_rebuild.json   # the workflow the blog walks through (simplified, ~22 nodes)
│   └── RMA_Copilot.json              # full production flow (multi-turn, approval split, per-branch state)
└── README.md
```

> If you installed the APEX app **with Supporting Objects**, the `sql/` scripts are already applied for you — they're here for reference and for anyone who'd rather run them by hand.
...
### `APP_CONFIG` — non-secret settings
...
### Secrets — use the APEX Web Credential store, **not** a config row

APEX-side outbound (a human agent replying from inside the app) needs your Twilio credentials. Twilio uses HTTP Basic auth, so store them as a **Web Credential**, type *Basic Authentication*:

- **Static ID:** `TWILIO`
- **Username:** Account SID
- **Password:** Auth Token

Web Credentials are write-only, encrypted, and excluded from app exports by default — so the token can't leak through a SQL dump or a shared export. n8n keeps its *own* Twilio credential in n8n's encrypted store.
...
| Method | Path | Purpose |
|---|---|---|
| `POST`  | `/oauth/token` | Issue an access token (OAuth client credentials) |
...
- Keep Twilio credentials in the APEX Web Credential store, never in `APP_CONFIG` rows or committed SQL.

---

## Links

- 📖 Blog: [When an APEX App Talks Back](ADD-YOUR-BLOG-LINK-HERE)
- 🎤 Talk: *When APEX Talks Back* (Kscope)
